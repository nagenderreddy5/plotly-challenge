...
function build_barchart(sample) {
    d3.json("samples.json").then((data) => {
        ...
        // plot a bar chart
        Plotly.newPlot("bar", BAR_chart_trace1, BAR_chart_layout);
    }); 
} 
...
function build_bubble(sample) {
    d3.json("samples.json").then((data) => {
        ...
        // plot bar chart
        Plotly.newPlot("bubble", bubble_chart_trace1, bubble_chart_layout);
...
function build_meta(sample) {
    d3.json("samples.json").then((data) => {
        ...
        var miniJSON2 = metadata.filter(sampleID => sampleID.id == sample);
        var object2 = miniJSON2[0];
